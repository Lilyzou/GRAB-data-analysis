...
wantROI = input('Custom ROI? (Y/N): ','s');
if (strcmp(wantROI,'Y')) || (strcmp(wantROI,'y'))
    roiX1 = input('X1: ');
    roiX2 = input('X2: ');
    roiY1 = input('Y1: ');
    roiY2 = input('Y2: ');
...
timeNormData = cell(length(fileNames),21); %21 represents the number of hm arrays that are generated in each loop
frameNormData = cell(length(fileNames),21);
baseData = cell(length(fileNames),21);
...
for sessionLoop = 1:length(fileNames)
    ...
    roiName = strcat('ROI_', int2str(roiX1), '_', int2str(roiX2), '_', int2str(roiY1), '_', int2str(roiY2));
    mkdir(roiName);
    cd(strcat(pwd,'\',roiName, '\'));
    ...
    cd('..');
    currFolder = pwd;
    baseMeanFolder = strcat(currFolder,'\',roiName,'\trial-data-mean-base\');
    normMeanFolder = strcat(currFolder,'\',roiName,'\trial-data-mean-16frames\');
    timeMeanFolder = strcat(currFolder,'\',roiName,'\trial-data-mean-500ms\');
    moveFolder1 = strcat(currFolder,'\',roiName,'\movement-analysis\');
    moveFolder2 = strcat(currFolder,'\',roiName,'\movement-analysis-16frames\');
    moveFolder3 = strcat(currFolder,'\',roiName,'\movement-analysis-500ms\');
    whiskname = strcat('AH',fname(1:4),fname(6:9),'-WTLIA.mat');
    loadname = strcat(fname(6:9),'load-ROI.mat');
    if isfile(loadname) %%%%%%%%%%%%%%%IF YOU CHANGE ANYTHING WITHIN THIS IF ELSE BLOCK, MAKE SURE TO DELETE THE LOAD FILE SO THAT YOU CAN GENERATE A NEW FILE
        ...
        if isfile(strcat(fname,'_rigid.mat'))
            ...
            if (roiX1 < 0) || (roiX2 < 0) || (roiY1 < 0) || (roiY2 < 0)
                z1=squeeze(x1(1,:,:,:));
            else
                z1=squeeze(x1(1,roiX1:roiX2,roiY1:roiY2,:));%(choose the ROI at 2nd and 3rd dimension)
            end
    ...
    n = n+1;
    figure(n);clf
    imagesc(squeeze(mean(y(1,roiX1:roiX2,roiY1:roiY2,:),4))), truesize, axis off
    colormap(gray)
...
comboSave = strcat(comboSave,'-ROI.mat');
save(comboSave);
